...
export class JanProvider extends BaseProvider {
  ...
  private static readonly DEFAULT_BASE_URL = 'http://127.0.0.1:1337/v1';
  ...
  async sendMessage(
    ...
    let baseUrl = settings.baseUrl || provider.baseUrl || JanProvider.DEFAULT_BASE_URL;
  ...
  private async getJanTools(settings: LLMSettings): Promise<unknown[]> {
    try {
      ...
      return this.formatToolsForJan(rawTools);
    } catch {
      return [];
    }
  }
  ...
  async fetchModels(apiKey: string, baseUrl?: string): Promise<string[]> {
    ...
    let janBaseUrl = baseUrl && baseUrl.trim() ? baseUrl.trim() : JanProvider.DEFAULT_BASE_URL;
    ...
    for (const hostUrl of hostsToTry) {
      try {
        ...
        return data.data.map((model: { id?: string; name?: string }) => model.id || model.name).filter(Boolean);
...
Use these tools when appropriate to help answer the user's questions or complete tasks.`;
  ...
  /**
   * Create usage information from Jan AI API response (no cost calculation for local provider)
   */
